...
static void shiftrows(uint64_t* state) {
	...
	for(int i = 0; i < 8; i++)
	{
		SWAPMOVE(state[i], state[i], 0x00f000000f0f0000, 4);
		SWAPMOVE(state[i], state[i], 0x000000ff00000000, 8);
		SWAPMOVE(state[i], state[i], 0x000f000000000000, 12);
	}
}

static void mixcolumns_0(uint64_t* state) {
	uint64_t t0, t1, t2, t3, t4;
	uint64_t temp1[8], temp2[8];
	for(int i = 0; i < 8; i++)
	{
		temp1[i] = state[i] & 0xf0f00f0ff0f00f0f;
		temp2[i] = state[i] & 0x0f0ff0f00f0ff0f0;
	}
	t3 = ROR(BYTE_ROR_4(temp1[0]),16);
	t0 = temp1[0] ^ t3;
	t1 = ROR(BYTE_ROR_4(temp1[7]),16);
	t2 = temp1[7] ^ t1;
	temp1[7] = ROR(BYTE_ROR_8(t2),32) ^ t1 ^ t0;
	t1 = ROR(BYTE_ROR_4(temp1[6]),16);
	t4 = t1 ^ temp1[6];
	temp1[6] = t2 ^ t0 ^ t1 ^ ROR(BYTE_ROR_8(t4),32);
	t1 = ROR(BYTE_ROR_4(temp1[5]),16);
	t2 = t1 ^ temp1[5];
	temp1[5] = t4 ^ t1 ^ ROR(BYTE_ROR_8(t2),32);
	t1 = ROR(BYTE_ROR_4(temp1[4]),16);
	t4 = t1 ^ temp1[4];
	temp1[4] = t2 ^ t0 ^ t1 ^ ROR(BYTE_ROR_8(t4),32);
	t1 = ROR(BYTE_ROR_4(temp1[3]),16);
	t2 = t1 ^ temp1[3];
	temp1[3] = t4 ^ t0 ^ t1 ^ ROR(BYTE_ROR_8(t2),32);
	t1 = ROR(BYTE_ROR_4(temp1[2]),16);
	t4 = t1 ^ temp1[2];
	temp1[2] = t2 ^ t1 ^ ROR(BYTE_ROR_8(t4),32);
	t1 = ROR(BYTE_ROR_4(temp1[1]),16);
	t2 = t1 ^ temp1[1];
	temp1[1] = t4 ^ t1 ^ ROR(BYTE_ROR_8(t2),32);
	temp1[0] = t2 ^ t3 ^ ROR(BYTE_ROR_8(t0),32);
	t3 = ROR(BYTE_ROR_12(temp2[0]),16);
	t0 = temp2[0] ^ t3;
	t1 = ROR(BYTE_ROR_12(temp2[7]),16);
	t2 = temp2[7] ^ t1;
	temp2[7] = ROR(BYTE_ROR_8(t2),32) ^ t1 ^ t0;
	t1 = ROR(BYTE_ROR_12(temp2[6]),16);
	t4 = t1 ^ temp2[6];
	temp2[6] = t2 ^ t0 ^ t1 ^ ROR(BYTE_ROR_8(t4),32);
	t1 = ROR(BYTE_ROR_12(temp2[5]),16);
	t2 = t1 ^ temp2[5];
	temp2[5] = t4 ^ t1 ^ ROR(BYTE_ROR_8(t2),32);
	t1 = ROR(BYTE_ROR_12(temp2[4]),16);
	t4 = t1 ^ temp2[4];
	temp2[4] = t2 ^ t0 ^ t1 ^ ROR(BYTE_ROR_8(t4),32);
	t1 = ROR(BYTE_ROR_12(temp2[3]),16);
	t2 = t1 ^ temp2[3];
	temp2[3] = t4 ^ t0 ^ t1 ^ ROR(BYTE_ROR_8(t2),32);
	t1 = ROR(BYTE_ROR_12(temp2[2]),16);
	t4 = t1 ^ temp2[2];
	temp2[2] = t2 ^ t1 ^ ROR(BYTE_ROR_8(t4),32);
	t1 = ROR(BYTE_ROR_12(temp2[1]),16);
	t2 = t1 ^ temp2[1];
	temp2[1] = t4 ^ t1 ^ ROR(BYTE_ROR_8(t2),32);
	temp2[0] = t2 ^ t3 ^ ROR(BYTE_ROR_8(t0),32);
	for(int i = 0; i < 8; i++)
...
void AES128_SR1_enc(unsigned char* ctext0, unsigned char* ctext1, 
	...
	mixcolumns_0(state); 								// 3rd round
	ark(state, rkeys_ffs + 24);							// 3rd round

	sbox(state); 										// 4th round
	mixcolumns_1(state); 								// 4th round
	...
	mixcolumns_0(state); 								// 7th round
	ark(state, rkeys_ffs + 56); 						// 7th round

	sbox(state); 										// 8th round
	mixcolumns_1(state); 								// 8th round
	...
	//shiftrows(state); 								// 10th round (resynchronization)	
